...
def molecular_dynamics(
    ...
    min_dist = part_props[
        "min_dist"
    ]  # minimum distance between particles in initialization
    ...
    if plot == True:
        ...
        plt.plot(time_out, Ek, label="Kinetic", c="C0")
        plt.plot(time_out, Ep, label="Potential", c="C3")
        plt.plot(time_out, E, label="Total", c="C4")
        plt.ylabel("Energy")
        plt.xlabel("Time")
        ...
        temp = plt.plot(time_out[5:], T[5:], label="Temperature", c="C0")
        plt.ylabel("Temperature")
        plt.xlabel("Time")
        ax_r = ax_l.twinx()
        pres = plt.plot(time_out[5:], P[5:], label="Pressure", c="C3")
        plt.ylabel("Pressure")
        lines = temp + pres
        ...
        plt.tight_layout()
        plt.savefig(sim_name + " - image.png", dpi=300)
    ...
    if save_csv == True:
        ...
        time_df = pd.DataFrame(
            time_out, columns=pd.MultiIndex.from_tuples([("t", "t")])
        )
...
if __name__ == "__main__":
    ...
    part_props = {"epsilon": 1, "sigma": 1, "mass": 1, "density": 1, "min_dist": 0.8}
    time_props = {
        "delta_t": 0.0001,
        "timesteps": 1000,
    ...
    _,_,_,_,_,_,T, P = molecular_dynamics(
        part_props,
        "Canonical",
        box_size,
        time_props,
